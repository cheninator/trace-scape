...
export class DisksIOModelProviderTest extends BaseXYModelProviderTest {
    ...
    @test("queries the xy entries to the Disks IO endpoint from trace start to trace end", timeout(50000))
    public async query_entries() {
        await this.queryEntries(100, 20, this.testTitle);
    }

    @test("queries 10 xy points to the Disks IO endpoint from trace start to trace end", timeout(50000))
    public async query_10_points() {
        await this.queryXY(10, 20, this.testTitle);
    }

    @test("queries 100 xy points to the Disks IO endpoint from trace start to trace end", timeout(50000))
    public async query_100_points() {
        await this.queryXY(100, 20, this.testTitle);
    }

    @test("queries 1000 xy points to the Disks IO endpoint from trace start to trace end", timeout(500000))
    public async query_1000_points() {
        await this.queryXY(1000, 10, this.testTitle);
    }

    @test("queries 10000 xy points to the Disks IO endpoint from trace start to trace end", timeout(5000000))
